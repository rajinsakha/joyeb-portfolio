...
export const FloatingNav = ({
  ...
  const [activeSection, setActiveSection] = useState(navItems[0].link); // Set initial state to Home section's link
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      for (let i = 0; i < offsets.length; i++) {
        if (scrollPos >= offsets[i] && (!offsets[i + 1] || scrollPos < offsets[i + 1])) {
      ...
      >
        {navItems.map((navItem, idx) => (
          <Link
            ...
            href={navItem.link}
            className={cn(
              "relative dark:text-neutral-50 items-center flex space-x-1 text-neutral-600 dark:hover:text-neutral-300 hover:text-neutral-500 px-4 py-3",
              {
                'bg-[#1A1A1A] text-white rounded-full': activeSection === navItem.link
              }
            )}
